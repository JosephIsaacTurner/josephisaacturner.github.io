class PapayaSegmenter {
    ...
    lassoTool() {
        ...
        const drawPath = (points, ctx) => {
            ...
            for (var i = 1; i < points.length; i++) {
                let testPoint = {
                    // To swap axial to coronal
                    // screen coord x = x in axial, 
                    // and screen coord x = x in coronal
                    // BUT, y = y in axial, and y = z in coronal
                    // z = z in axial (cursorPosition.z)
                    // z = y in coronal (cursorPosition.y)
                    x: this.viewer.convertScreenToImageCoordinateX(points[i].x, this.viewer.coronalSlice),
                    z: this.viewer.convertScreenToImageCoordinateY(points[i].y, this.viewer.coronalSlice),
                    y: this.viewer.cursorPosition.z
                };
    ...
    addToLassoList(lassoPoints) {
        ...
        lassoPoints.forEach(point => {
            ...
            let newPoint = {
                x: this.viewer.convertScreenToImageCoordinateX(point[0], this.viewer.coronalSlice),
                z: this.viewer.convertScreenToImageCoordinateY(point[1], this.viewer.coronalSlice),
                y: cursorPosition.z
            };
